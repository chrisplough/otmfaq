...
($hook = vBulletinHook::fetch_hook('post_groan_main_start')) ? eval($hook) : false;

if ($_REQUEST['do'] == 'post_groan_add')
{
	($hook = vBulletinHook::fetch_hook('post_groan_main_add_groan_start')) ? eval($hook) : false;

	$postinfo = array_merge($postinfo, fetch_userinfo($postinfo['userid']));
	if (post_groan_off($forumid, $postinfo, $threadinfo['firstpostid']) || !can_groan_this_post($postinfo, $threadinfo['isdeleted']) || groaned_already($postinfo))
	{
	...
	($hook = vBulletinHook::fetch_hook('post_groan_main_add_groan_end')) ? eval($hook) : false;

	if ($using_ajax)
	{
		$groans = fetch_groans($postid, '', true);

		$postinfo['post_groan_bit'] = fetch_groan_bit($forumid, $groans);
		
		$postinfo['post_groan_amount'] = $postinfo['post_groan_amount'] + 1;
		$postinfo['post_groan_amount_formatted'] = vb_number_format($postinfo['post_groan_amount']);

		if ($vbulletin->options['post_groan_delete_own'])
		{
			$postinfo['show_groan_remove_option'] = true;
		}

		$echo = fetch_post_groan_template($postinfo);
		echo "$echo";
		exit;
	}
	else
	{
		...
		eval(print_standard_redirect('redirect_post_groan'));
	}
}

if ($_REQUEST['do'] == 'post_groan_remove_all')
{
	($hook = vBulletinHook::fetch_hook('post_groan_main_remove_all_groan_start')) ? eval($hook) : false;

	if (!(can_delete_all_groans()))
	{
		$using_ajax ? exit : print_no_permission();
	}

	delete_all_groans($postinfo);

	($hook = vBulletinHook::fetch_hook('post_groan_main_remove_all_groan_end')) ? eval($hook) : false;
...
if ($_REQUEST['do'] == 'post_groan_remove_user')
{
	($hook = vBulletinHook::fetch_hook('post_groan_main_remove_user_groan_start')) ? eval($hook) : false;

	if (!(delete_groan($postinfo, $vbulletin->userinfo['userid'])))
	{
		$using_ajax ? exit : print_no_permission();
	}	

	($hook = vBulletinHook::fetch_hook('post_groan_main_remove_user_groan_end')) ? eval($hook) : false;

	$postinfo['post_groan_amount'] = $postinfo['post_groan_amount'] - 1;

	if ($postinfo['post_groan_amount'] > 0 && $using_ajax)
	{     
		$groans = fetch_groans($postid, '', true);
		...
		$postinfo['post_groan_amount_formatted'] = vb_number_format($postinfo['post_groan_amount']);
		$postinfo['post_groan_user'] = false;
		$postinfo['ajax'] = true;
...
if ($_REQUEST['do'] == 'findallgroans')
{
	...
	else
	{
		...
		$posts = $db->query_read_slave("
			...
			FROM " . TABLE_PREFIX . "post AS post
			INNER JOIN " . TABLE_PREFIX . "thread AS thread ON(thread.threadid = post.threadid)
			WHERE post.post_groan_amount != 0
				AND thread.forumid IN(" . implode(',', $forumids) . ")
			ORDER BY post.dateline DESC
			LIMIT " . ($vbulletin->options['maxresults'] * 2) . "
		");
...
($hook = vBulletinHook::fetch_hook('post_groan_main_end')) ? eval($hook) : false;
?>