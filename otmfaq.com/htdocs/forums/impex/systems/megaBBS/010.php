...
class megaBBS_010 extends megaBBS_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_name_array["$pm[toname]"];
			$username	= $pm['toname'];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_name_array["$pm[fromname]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_name_array["$pm[toname]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_name_array["$pm[fromname]"]);
...
|| # CVS: $RCSfile: 010.php,v $ - $Revision: 1.7 $
|| ####################################################################
\*======================================================================*/
?>
