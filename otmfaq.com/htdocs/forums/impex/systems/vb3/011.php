...
class vb3_011 extends vb3_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $details)
		{
			...
			{
				$touserarray[$users_ids[$old_user_id]] = $username;
			}

			// Mandatory
			$pm_text->set_value('mandatory', 'fromuserid',			$users_ids["$details[fromuserid]"]);
			...
			if($pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					foreach ($pms as $pm => $details)
					{
						...
						$pm->set_value('mandatory', 'userid',			$users_ids["$details[userid]"]);
...
|| # CVS: $RCSfile: 011.php,v $ - $Revision: 1.14 $
|| ####################################################################
\*======================================================================*/
?>
