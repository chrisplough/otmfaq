	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('nonarchivefinished') == 'FALSE') # NOW DO THE NORMAL POSTS
		{
			...
			foreach ($post_array as $post_id => $post_details)
			{
				...
				if (!$thread_ids_array["$post_details[TOPIC_OID]"])
				{
					$missing_thread = true;
					// Deleted thread ?
				}
				...
				// Mandatory
				$try->set_value('mandatory', 'threadid',			$thread_ids_array["$post_details[TOPIC_OID]"]);
				$try->set_value('mandatory', 'userid',				$user_ids_array["$post_details[AUTHOR_OID]"]);
				$try->set_value('mandatory', 'importthreadid',		$post_details['TOPIC_OID']);
	
				// Non Mandatory
				$try->set_value('nonmandatory', 'parentid',			'0');
				$try->set_value('nonmandatory', 'username',			$user_name_array["$post_details[AUTHOR_OID]"]);
				$try->set_value('nonmandatory', 'dateline',			@strtotime($post_details['DATETIME_CREATED']));
				$try->set_value('nonmandatory', 'pagetext',			$this->eve_html($this->html_2_bb($post_details['BODY'])));
				...
				$try->set_value('nonmandatory', 'visible',			$post_details['IS_MESSAGE_VISIBLE']);
				$try->set_value('nonmandatory', 'importpostid',		$post_details['MESSAGE_OID']);
				...
				else
				{
					if ($missing_thread)
					{
						$displayobject->display_now("<br />{$displayobject->phrases['invalid_object']} missing thread, most likely a Private Message" . $post_details['TOPIC_OID']);
					}
					else
					{
		...
		else # NOW DO THE ARCHIVE POSTS
		{
			...
			foreach ($post_array as $post_id => $post_details)
			{
				...
				$try->set_value('nonmandatory', 'showsignature',	'1');
				$try->set_value('nonmandatory', 'ipaddress',		$post_details['POSTER_IP']);
				$try->set_value('nonmandatory', 'visible',			$post_details['IS_VISIBLE']);
				$try->set_value('nonmandatory', 'importpostid',		$post_details['MESSAGE_OID']);
...
|| # CVS: $RCSfile: 006.php,v $ - $Revision: 1.32 $
|| ####################################################################
\*======================================================================*/
?>
