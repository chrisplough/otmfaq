...
class eve_003 extends eve_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($user_array as $user_id => $user)
		{
			...
			if ($sessionobject->get_session_var('username_email'))
			{
				$try->set_value('mandatory', 'username',			addslashes($user['DISPLAY_NAME']));
			}
			else
			{
			...
			$try->add_default_value('Biography', 					addslashes(substr($user_details['BIO'], 0, 250)));
...
|| # CVS: $RCSfile: 003.php,v $ - $Revision: 1.41 $
|| ####################################################################
\*======================================================================*/
?>
