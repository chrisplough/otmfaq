	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
			...
			foreach ($subscription_array as $subscription_id => $subscription_details)
			{
				...
				if($try->is_valid())
				{
					...
					else
					{
						...
						$sessionobject->add_error($subscription_id, $displayobject->phrases['subscription_not_imported'], $displayobject->phrases['subscription_not_imported_rem']);
		...
		else
		{
			...
			foreach ($subscriptionlog_array as $subscriptionlog_id => $subscriptionlog_details)
			{
				...
				$try->set_value('mandatory', 'userid', 					$users_ids["$subscriptionlog_details[userid]"]);
...
|| # CVS: $RCSfile: 014.php,v $ - $Revision: 1.2 $
|| ####################################################################
\*======================================================================*/
?>
