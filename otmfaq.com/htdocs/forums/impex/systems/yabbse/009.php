...
class yabbse_009 extends yabbse_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm_details)
		{
			...
			$userid 	= $user_ids_array["$pm_details[ID_MEMBER_TO]"];
			$username	= $user_names["$pm_details[ID_MEMBER_TO]"];
			unset($touserarray);

			$touserarray[$userid] = $user_names["$pm_details[ID_MEMBER_TO]"];

			 // Mandatory
			$vB_pm_text->set_value('mandatory', 'fromuserid',				$user_ids_array["$pm_details[ID_MEMBER_FROM]"]);
			...
			// Non Mandatory
			$vB_pm_text->set_value('nonmandatory', 'fromusername',			$user_names["$pm_details[ID_MEMBER_FROM]"]);
			$vB_pm_text->set_value('nonmandatory', 'dateline',				$pm_details['msgtime']);
				...
				if($vB_pm_text->is_valid())
				{
					...
					if($pm_text_id)
					{
						...
						$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm_details[ID_MEMBER_TO]"]);
						...
						$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm_details[ID_MEMBER_FROM]"]);
						...
						if($vB_pm_to->is_valid() AND $vB_pm_from->is_valid())
						{
							...
							{
								$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $user_names[$pm_details['ID_MEMBER_FROM']] . '</b> , to : <b>' . $user_names[$pm_details['ID_MEMBER_TO']] . '</b>');
...
|| # CVS: $RCSfile: 009.php,v $ - $Revision: 1.8 $
|| ####################################################################
\*======================================================================*/
?>
