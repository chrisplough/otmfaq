...
class fusetalk_008 extends fusetalk_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids_array["$pm[reciverid]"];
			$username	= $user_name_array["$pm[reciverid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[senderid]"]);
			if($pm['mt_title'])
			{
			...
			$vB_pm_text->set_value('mandatory', 'importpmid',		$pm['imessageid']);

			$vB_pm_text->set_value('mandatory', 'touserarray',		addslashes(serialize($touserarray)));
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[senderid]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[reciverid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[senderid]"]);
...
|| # CVS: $RCSfile: 008.php,v $ - $Revision: 1.10 $
|| ####################################################################
\*======================================================================*/
?>
