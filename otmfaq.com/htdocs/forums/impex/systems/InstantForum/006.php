...
class InstantForum_006 extends InstantForum_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($thread_array as $thread_id => $thread_details)
		{
			...
			$try->set_value('nonmandatory', 'sticky',		$thread_details['PinnedPost']);
			$try->set_value('nonmandatory', 'views',		$thread_details['Views']);
			$try->set_value('nonmandatory', 'open',			'1');
			$try->set_value('nonmandatory', 'replycount',	$thread_details['NoOfReplies']);
			$try->set_value('nonmandatory', 'postusername',	$user_name_array["$thread_details[UserID]"]);
			$try->set_value('nonmandatory', 'postuserid',	$user_ids_array["$thread_details[postuserid]"]);
			$try->set_value('nonmandatory', 'dateline',		strtotime($thread_details['PostedDate']));
