...
class InstantForum_004 extends InstantForum_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'email',				$user_details['Username']);
			...
			$try->set_value('mandatory', 'username',			$user_details['FullName']);
			...
			$try->set_value('nonmandatory', 'msn',				$user_details['MSNIM']);
			...
			$try->set_value('nonmandatory', 'password',			$user_details['Password']); // Just to set a random string
			$try->set_value('nonmandatory', 'yahoo',			$user_details['YahooIM']);
			$try->set_value('nonmandatory', 'usertitle',		$user_details['UserStatus']);
			$try->set_value('nonmandatory', 'posts',			$user_details['NoOfPosts']);
