...
class InstantForum_008 extends InstantForum_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[RecipientMemberID]"];
			$username	= $user_names["$pm[RecipientMemberID]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[SenderMemberID]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['privmsgs_subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($this->InstantForum_html($pm_text['Message'])));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[SenderMemberID]"]);
			$vB_pm_text->set_value('nonmandatory', 'iconid',		'');
			$vB_pm_text->set_value('nonmandatory', 'dateline',		strtotime($pm['PostedDate']));
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$users_ids["$pm[RecipientMemberID]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[SenderMemberID]"]);
