	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		if($sessionobject->get_session_var('categoriesfinished') == 'FALSE')
		{
			...
			foreach ($categories_array as $cat_id => $cat)
			{
				...
				$try->set_value('mandatory', 'title', 				$cat['GroupName']);
				$try->set_value('mandatory', 'displayorder',		$cat['GroupOrder']);
				...
				$try->set_value('nonmandatory', 'description', 		$cat['GroupName']);
		...
		else
		{
			...
			foreach ($forum_array as $forum_id => $forum_details)
			{
				...
				$try->set_value('mandatory', 'displayorder',		$forum_details['ForumSortOrder']);
				$try->set_value('mandatory', 'parentid',			$cat_ids["$forum_details[GroupID]"]);
	
				// Non Mandatory
				$try->set_value('nonmandatory', 'description',		$forum_details['ForumDescription']);
				$try->set_value('nonmandatory', 'threadcount',		$forum_details['NumberOfTopics']);
				$try->set_value('nonmandatory', 'replycount',		$forum_details['NumberOfReplies']);
