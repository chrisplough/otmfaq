...
class InstantForum_007 extends InstantForum_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($post_array as $post_id => $post_details)
		{
			...
			// Mandatory
			if ($thread_ids_array["$post_details[TopicID]"])
			{
				$try->set_value('mandatory', 'threadid',		$thread_ids_array["$post_details[TopicID]"]);
			}
			else
			{
				if(impexdebug) {echo  "<h1>'" . $post_details['TopicID'] . "'</h1>"; }
				if(impexdebug) {echo  "<h1>'" . $thread_ids_array["$post_details[TopicID]"] . "'</h1>"; }
			}
			...
			$try->set_value('nonmandatory', 'dateline',		strtotime($post_details['PostedDate']));
