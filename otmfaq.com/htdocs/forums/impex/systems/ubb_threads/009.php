...
class ubb_threads_009 extends ubb_threads_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $users_ids["$pm[M_Uid]"];
			$username	= $user_names["$pm[M_Uid]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[M_Sender]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[M_Uid]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[M_Sender]"]);
