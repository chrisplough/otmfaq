...
class edge_003 extends edge_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			$try->add_custom_value('fullname', 					addslashes($user_details['name']));
			$try->add_custom_value('likes', 					addslashes($this->html_2_bb($user_details['likes'])));
			$try->add_custom_value('dislikes', 					addslashes($this->html_2_bb($user_details['dislikes'])));
			$try->add_custom_value('address', 					addslashes($this->html_2_bb($user_details['address'])));
