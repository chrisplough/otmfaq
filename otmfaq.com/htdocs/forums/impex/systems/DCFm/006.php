...
class DCFm_006 extends DCFm_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($threads_array as $thread_id => $thread)
		{
			...
			$try->set_value('nonmandatory', 'postusername',	$user_names["$thread[author_id]"]);
			$try->set_value('nonmandatory', 'postuserid', 	$users_ids["$thread[topic_poster]"]);
			...
			if($try->is_valid())
			{
				...
				else
				{
					$sessionobject->add_error('warning',
											 $this->_modulestring,
											 get_class($this) . "::import_thread failed for " . $cat['cat_title'] . " get_DCFm_categories_details was ok.",
											 'Check database permissions and user table');
...
|| # CVS: $RCSfile: 006.php,v $ - $Revision: 1.11 $
|| ####################################################################
\*======================================================================*/
?>
