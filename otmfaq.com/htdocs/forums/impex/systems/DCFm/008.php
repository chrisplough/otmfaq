				...
				if($vB_pm_text->is_valid())
				{
					...
					if($pm_text_id)
					{
						...
						if($vB_pm_to->is_valid() AND $vB_pm_from->is_valid())
						{
							...
							{
								$displayobject->display_now('<br /><font color="green">' . $vB_pm_to->how_complete() . '% </font>Imported from : ' . $user_names["$pm[from_id]"] . ' <b>to</b> ' . $user_names["$pm[to_id]"] . '');
...
|| # CVS: $RCSfile: 008.php,v $ - $Revision: 1.8 $
|| ####################################################################
\*======================================================================*/
?>
