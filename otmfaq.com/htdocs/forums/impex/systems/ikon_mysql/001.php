...
/*
// Can we row count so if > X do this less not ?
		if(!$sessionobject->get_session_var('attachindexed'))
		{
			$sessionobject->add_session_var('attachindexed');
			// Altering source post table to index on attachment id so the attachement import dosn't take for ever on big boards
			$alter_result = $this->alter_table_for_attachments($Db_source, $source_db_type, $source_table_prefix);

			if($alter_result)
			{
				$displayobject->display_now('<p>Source post table altered OK.</p>');
			}
			else
			{
				$displayobject->display_now('<p>Source post table NOT altered, import will work though attachments may take a LONG time.</p>');
			}
		}
*/
...
|| # CVS: $RCSfile: 001.php,v $ - $Revision: 1.11 $
|| ####################################################################
\*======================================================================*/
?>
