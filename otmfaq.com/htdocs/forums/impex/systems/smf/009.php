...
class smf_009 extends smf_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pmtext_id => $pm)
		{
			...
			{
				$userid_1 	= $user_ids_array["$im_recipients[ID_MEMBER]"];
				$username_1	= $user_name_array["$im_recipients[ID_MEMBER]"];
				$userid_2 	= $user_ids_array["$im_recipients[bcc]"];
				$username_2	= $user_name_array["$im_recipients[bcc]"];
				$touserarray[$userid_1] = $username_1;
				$touserarray[$userid_2] = $username_2;
			}
			else
			{
			...
			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[ID_MEMBER_FROM]"]);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[ID_MEMBER_FROM]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$im_recipients[ID_MEMBER]"]);
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[ID_MEMBER_FROM]"]);
...
|| # CVS: $RCSfile: 009.php,v $ - $Revision: 1.15 $
|| ####################################################################
\*======================================================================*/
?>
