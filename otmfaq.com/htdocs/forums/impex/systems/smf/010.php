...
class smf_010 extends smf_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($moderator_array as $moderator_id => $moderator_details)
		{
			...
			// Mandatory
			$try->set_value('mandatory', 'userid',				$user_ids_array["$moderator_details[ID_MEMBER]"]);
			...
			if($try->is_valid())
			{
				...
				{
					$displayobject->display_now('<br /><span class="isucc"><b>' . $try->how_complete() . '%</b></span> ' . $displayobject->phrases['moderator'] . ' -> ' . $user_name_array["$moderator_details[ID_MEMBER]"]);
