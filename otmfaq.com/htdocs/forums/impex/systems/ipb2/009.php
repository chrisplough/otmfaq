...
class ipb2_009 extends ipb2_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($pm_array as $pm_id => $pm)
		{
			...
			$userid 	= $user_ids_array["$pm[mt_to_id]"];
			$username	= $user_name_array["$pm[mt_to_id]"];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_ids_array["$pm[mt_from_id]"]);
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_name_array["$pm[mt_from_id]"]);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					$vB_pm_from->set_value('mandatory', 'userid',			$user_ids_array["$pm[mt_from_id]"]);
					...
					$vB_pm_to->set_value('mandatory', 'userid',				$user_ids_array["$pm[mt_to_id]"]);
				...
				{
					$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num. '_objects_failed') + 1 );
					$sessionobject->add_error($pm_text_id, $displayobject->phrases['pm_not_imported'], $displayobject->phrases['pm_not_imported_rem_2']);
					$displayobject->display_now("<br />{$displayobject->phrases['failed']} :: {$displayobject->phrases['pm_not_imported']}");
					$sessionobject->set_session_var($class_num . '_objects_failed',$sessionobject->get_session_var($class_num . '_objects_failed') + 1 );				
				}
			}
...
|| # CVS: $RCSfile: 009.php,v $ - $Revision: 1.12 $
|| ####################################################################
\*======================================================================*/
?>
