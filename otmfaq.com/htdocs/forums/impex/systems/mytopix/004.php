...
class mytopix_004 extends mytopix_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		foreach ($user_array as $user_id => $user_details)
		{
			...
			{
				$try->set_value('nonmandatory', 'birthday',			$user['members_birth_month'] . "-" . $user['members_birth_day'] . "-" . $user['members_birth_year']);
				$try->set_value('nonmandatory', 'birthday_search',	$user['members_birth_year'] . "-" . $user['members_birth_month'] . "-" . $user['members_birth_day']);
			}
