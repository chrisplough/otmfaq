	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($sessionobject->get_session_var('posts') == 'working')
		{
			...
			foreach ($posts_array as $post_id => $post)
			{
				...
				$try->set_value('mandatory', 'threadid', 			$thread_ids["$post[threadid]"]);
				...
				if($try->is_valid())
				{
					...
					else
					{
						$sessionobject->add_error('warning',
												 $this->_modulestring,
												 get_class($this) . "::import_thread failed for " . $cat['cat_title'] . " get_wbb_post was ok.",
												 'Check database permissions and user table');
