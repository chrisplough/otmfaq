	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		if ($sessionobject->get_session_var('pms') == 'working')
		{
			...
			foreach ($pms_array as $pm_id => $pm)
			{
				...
				$vB_pm_text->set_value('mandatory', 'fromuserid',		$users_ids["$pm[senderid]"]);
				...
				$vB_pm_text->set_value('nonmandatory', 'fromusername',	$user_names["$pm[senderid]"]);
				...
				if($vB_pm_text->is_valid())
				{
					...
					if($pm_text_id)
					{
						...
						$vB_pm_to->set_value('mandatory', 'userid',				$users_ids[$recipt_id]);
						...
						$vB_pm_from->set_value('mandatory', 'userid',			$users_ids["$pm[senderid]"]);
						...
						if($vB_pm_text->is_valid())
						{
							...
							{
								$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_to->how_complete() . '% </span>Imported from : <b>' . $user_names["$pm[senderid]"] . '</b> , to : <b>' . $user_names[$recipt_id] . '</b>');
...
|| # CVS: $RCSfile: 011.php,v $ - $Revision: 1.16 $
|| ####################################################################
\*======================================================================*/
?>
