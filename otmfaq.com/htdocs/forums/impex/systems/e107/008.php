...
class e107_008 extends e107_000
{
	...
	function resume(&$sessionobject, &$displayobject, &$Db_target, &$Db_source)
	{
		...
		{
			if(!$pm_to_id = $user_name_array["$pm[pm_to_user]"])
			{
				$displayobject->display_now('<br />Can not match sent to name <b>"' . $pm['pm_to_user'] .'"</b> , skipping');
			...
			$userid 	= $pm_to_id;
			$username	= $pm['pm_to_user'];
			unset($touserarray);
			$touserarray[$userid] = $username;

			$vB_pm_text->set_value('mandatory', 'fromuserid',		$user_name_array["$pm[fromid]"]);
			$vB_pm_text->set_value('mandatory', 'title',			$pm['pm_subject']);
			$vB_pm_text->set_value('mandatory', 'message',			$this->html_2_bb($pm['pm_message']));
			...
			$vB_pm_text->set_value('nonmandatory', 'fromusername',		$pm['pm_from_user']);
			...
			if($vB_pm_text->is_valid())
			{
				...
				if($pm_text_id)
				{
					...
					if($vB_pm_text->is_valid())
					{
						...
						{
							$displayobject->display_now('<br /><span class="isucc"><b>' . $vB_pm_text->how_complete() . '%,</b></span> Imported from : <b>' . $pm['pm_from_user'] . '</b> , to : <b>' . $pm['pm_to_user'] . '</b>');
...
|| # CVS: $RCSfile: 008.php,v $ - $Revision: 1.5 $
|| ####################################################################
\*======================================================================*/
?>
