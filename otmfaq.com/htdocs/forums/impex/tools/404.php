...
if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}") === 0)
{
	switch ($old_system)
	{
		case 'phpBB2' :
			...
			{
				$action = 'forum';
				$sql = "SELECT forumid FROM {$tableprefix}forum WHERE importforumid = " . substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], '?f=')+3);
			}
			...
			if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}{$old_thread_script}") === 0)
			{
				...
				{
					$sql = "SELECT threadid FROM {$tableprefix}thread WHERE importthreadid = " . substr(substr($_SERVER['REQUEST_URI'], 2), 0,  strpos(substr($_SERVER['REQUEST_URI'], 2), '&'));
				}
				else
				{
					$sql = "SELECT threadid FROM {$tableprefix}thread WHERE importthreadid = " . substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], '?t=')+3);
				}  
			}
			...
			if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}{$old_post_script}") === 0)
			{
				...
				if( preg_match("/&/", $_SERVER['REQUEST_URI']) )
				{
					$sql = "SELECT postid FROM {$tableprefix}post WHERE importpostid = " . substr(substr($_SERVER['REQUEST_URI'], 2), 0,  strpos(substr($_SERVER['REQUEST_URI'], 2), '&'));
				}
				else
				{
					$sql = "SELECT postid FROM {$tableprefix}post WHERE importpostid = " . substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], '?p=')+3);
				}  
			}
			...
			if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}{$old_user_script}") === 0)
			{
				...
				$sql = "SELECT userid FROM {$tableprefix}user WHERE importuserid = " . substr(substr($_SERVER['REQUEST_URI'], 19), 0,  strpos(substr($_SERVER['REQUEST_URI'], 19), '&'));
			}
		break;
		...
		case 'ubb.threads' :
			...
			if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}{$old_forum_script}") === 0)
			{
				...
				$sql = "SELECT forumid FROM {$tableprefix}forum WHERE importforumid LIKE " . substr($_SERVER['REQUEST_URI'], strpos($_SERVER['REQUEST_URI'], 'Board=')+6);
			}			
			...
			if (strpos($_SERVER['REQUEST_URI'], "/{$old_folder}{$old_thread_script}") === 0)
			{
				...
				$sql = "SELECT threadid FROM {$tableprefix}thread WHERE importthreadid = " . substr($url, $begin+7, ($middle-$begin)-7); // +7 & -7 To remove the off set of Number=
