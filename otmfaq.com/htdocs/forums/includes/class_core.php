...
define('FILE_VERSION', '4.1.1'); // this should match installsteps.php
define('SIMPLE_VERSION', '411'); // see vB_Datastore::check_options()
...
*
* @package	vBulletin
* @version	$Revision: 54255 $
* @date		$Date: 2011-10-28 13:23:19 -0700 (Fri, 28 Oct 2011) $
*/
class vB_Database
{
	/**
					...
					switch ($vartype) {
						...
						case TYPE_NOHTMLCOND:
							...
							if ($lower_charset != 'utf-8')
							{
								...
								{
									$this->registry->GPC["$varname"] = ncrencode($this->registry->GPC["$varname"], true, false);
								}
								else
								{
...
//				$output_type = in_array($vbulletin->GPC['api'], array('xml', 'json')) ? $vbulletin->GPC['api'] : 'json';
	...
	protected static function remove_common_show(&$show)
	{
		...
		unset(
			...
			$show['communitylink'], $show['search_engine']
		);
	}
...
abstract class vB_Template_Data extends vB_Template
{
	...
	public function render($suppress_html_comments = false, $final = false)
	{
		...
		if ($final)
		{
			...
			if (function_exists('api_result_prewhitelist'))
			{
				$this->registered = api_result_prewhitelist($this->registered);
			}
			...
			if (function_exists('api_result_overwrite'))
			{
				$this->registered = api_result_overwrite($this->registered);
			}
...
class vB_Template_JSON extends vB_Template_Data
{
	...
	protected function render_output($suppress_html_comments = false)
	{
		...
		if ($lower_charset != 'utf-8')
		{
			...
			$this->to_utf8($this->registered, $lower_charset);
		}

		return json_encode($this->registered);
	}

	private function to_utf8(&$value, $charset)
	{
		if (is_array($value))
		{
			foreach ($value AS &$el)
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 09:55, Mon Sep 16th 2013
|| # CVS: $RCSfile$ - $Revision: 54255 $
|| ####################################################################
\*======================================================================*/
