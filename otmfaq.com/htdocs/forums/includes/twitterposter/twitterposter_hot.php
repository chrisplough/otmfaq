...
define('TWITTERPOSTER_HOTTHREAD', true);
...
if (!$vbulletin->options['twitter_hot_timespan'])
{
	echo 'Tweeting hot threads is disabled.';
	exit;
}
else
{
...
if ($threads_num = $vbulletin->db->num_rows($get_hot_threads))
{
...
while ($thread = $vbulletin->db->fetch_array($get_hot_threads))
{
	$threads["$thread[threadid]"] = $thread;
}

$vbulletin->db->free_result($get_hot_threads);
